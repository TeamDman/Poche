...
| `S-ROOM-002` | U33, U40, G19 | Invite holder | Room is initialized and open; valid unexpired unused invite for this room and signed stable key | Add connected unseated membership bound to the stable key in any open phase; consume one-time invite | `D-INVITE-INVALID`, `D-INVITE-EXPIRED`, `D-WRONG-ROOM`, `D-WRONG-PHASE`, or `D-REVOKED` | session-semantic |
...
| `S-ROOM-013` | U48, G23, G30 | Any game/environment actor | Paused | Chat, reconnect, grants, unpause, spectator admission, and unseated spectator leave remain possible; game/chance/settlement does not advance; a seated player's leave awaits an explicit game-dropout transition | Game/chance/settlement returns `D-PAUSED`; seated leave returns `D-WRONG-PHASE` | session-semantic |
...
| `S-ROOM-018` | U35, U40 | Non-host member | Unseated in any open phase, or Lobby/Countdown while seated | Remove active connection/seat/readiness; voluntary leave ends only the current membership; cancel countdown if affected. A later valid invite or browser room-code redemption creates a fresh unseated membership rather than restoring the old principal | Unknown principal is `D-UNKNOWN-PRINCIPAL`; seated active-game leave is `D-WRONG-PHASE` | session-semantic |
