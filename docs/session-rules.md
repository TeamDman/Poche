...
| `S-ROOM-013` | U48, G23, G30 | Any game/environment actor | Paused | Chat, reconnect, grants, and unpause remain possible; game/chance/settlement does not advance; destructive voluntary leave awaits an explicit game-dropout transition | Game/chance/settlement returns `D-PAUSED`; leave returns `D-WRONG-PHASE` | session-semantic |
...
| `S-ROOM-018` | U35, U40 | Member | Lobby or Countdown | Remove active connection/seat/readiness; voluntary leave revokes current membership epoch; cancel countdown if affected | Unknown principal is `D-UNKNOWN-PRINCIPAL`; active-game leave is `D-WRONG-PHASE` | session-semantic |
