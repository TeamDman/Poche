...
| `S-ROOM-002` | checked Tasks 2.2/5.2 invite codec/session units | abstracted Task 3.1: membership safety | abstracted Task 3.2: lifecycle | abstracted Task 3.3: invite query | checked Tasks 2.4/5.2 secret-free join/code vectors | checked Task 4.2 loopback join; Veilid native pending Tasks 5.2/5.6 | planned join/error |
...
| `S-ROOM-020` | checked Tasks 2.2/5.3 stable-key reconnect and removal denial | abstracted Task 3.1: key-bound member | checked Task 3.2 durable-member reconnect | queried Task 3.3 reconnect relation | checked Tasks 2.4/4.5/5.3 reconnect and recovery shapes | checked Tasks 4.5/5.3 restart, protected membership, and route proofs; native nodes Task 5.6 | planned reconnect status |
...
| `S-AUTH-013` | checked Tasks 2.2/5.1/5.3 stable public-key principal/restart | abstracted Task 3.1: stable-principal abstraction | abstracted Task 3.2: route-rotation | abstracted Task 3.3: identity query | checked Tasks 5.1/5.3 identity, membership, and reconnect signatures | checked Tasks 5.1/5.3 protected restart and replacement-route proof; native e2e Task 5.6 | planned identity display |
| `S-AUTH-014` | checked Task 5.2 compact version/network/expiry/checksum codec | abstracted Task 3.1: invite scope | abstracted Task 3.2: invite state | abstracted Task 3.3: issuance query | checked Task 5.2 canonical code vectors | checked Task 5.2 released DFLT adapter; native e2e pending | planned code display |
| `S-AUTH-015` | checked Tasks 2.2/5.2 invalid/replay/expiry/cross-room/revoked units | abstracted Task 3.1: key binding | abstracted Task 3.2: replay/expiry | abstracted Task 3.3: redemption query | checked Task 5.2 invite attacks | checked Task 5.2 stable-key reducer binding; native join pending | planned join reasons |
| `S-AUTH-016` | checked Tasks 5.1/5.4 event signing and strict reply ordering | N/A: unforgeability assumed | abstracted Task 3.2: signed-event flag | abstracted Task 3.3: provenance query | checked Tasks 5.1/5.4 canonical event/reply domains | checked Task 5.4 wire boundary; native tamper e2e Task 5.6 | planned integrity error |
...
| `S-AUTH-018` | checked Tasks 2.4/5.3 prefix snapshot/tail hashes and host signatures | abstracted Task 3.1: identity abstraction | abstracted Task 3.2: snapshot revision | abstracted Task 3.3: recovery query | checked Tasks 2.4/5.3 snapshot payload/signature vectors | checked Task 5.3 gap/reorder/wrong-host recovery boundary; transport e2e Task 5.4 | planned syncing state |
| `S-AUTH-019` | checked Tasks 2.1-2.4/5.1/5.3 non-diagnostic secret types and snapshot scans | N/A: secret bytes omitted by scope | N/A: secret bytes omitted by scope | abstracted Task 3.3: non-generation query | checked Tasks 2.4/5.1/5.3 public identity and secret-free membership shapes | checked Tasks 5.1/5.3 protected identity, membership, and DHT-owner blobs | planned redaction scan |
...
| `S-VIEW-012` | checked Task 2.3 immutable/future projection test | N/A: past knowledge cannot be erased | N/A: client memory outside session | abstracted Task 3.3: future-only query | checked Tasks 2.3/5.5 epoch/expiry and expected-epoch open | checked Task 5.5 future delivery stops; past knowledge disclaimer retained | planned cache removal |
| `S-VIEW-013` | checked Task 2.3 four-viewer controlled defect | checked Task 3.1 scoped-grant noninterference | abstracted Task 3.2: visibility safety | queried Task 3.3 matrix and broadcast defect | checked Tasks 2.3/5.5 typed projection/opaque delivery | checked Task 5.5 multi-view capture and wrong-recipient failures | planned DOM/widget scan |
...
| `S-FAULT-002` | checked Task 5.4 watch/route hint classifier | N/A: DHT outside model | abstracted Task 3.2: hint/non-authority flag | abstracted Task 3.3: refresh query | checked Task 5.4 explicit non-authoritative hint vocabulary | checked Task 5.4 released watch API; live renewal Task 5.6 | planned refreshing state |
| `S-FAULT-003` | checked Task 5.4 exact-command retry budget/categories | N/A: API errors outside model | abstracted Task 3.2: retry abstraction | abstracted Task 3.3: retry-category query | checked Task 5.4 retry disposition vectors | checked Task 5.4 released error mapping; forced network failures Task 5.6 | planned structured error |
| `S-FAULT-004` | checked Tasks 2.2/5.3 route-neutral reconnect and stable-key route signatures | abstracted Task 3.1: stable-key abstraction | checked Task 3.2 route-neutral disconnect/reconnect | abstracted Task 3.3: reconnect query | checked Task 5.3 host/member route epochs and protected locators | checked Task 5.3 boundary; native rotation e2e Task 5.6 | planned reconnect status |
| `S-FAULT-005` | checked Tasks 2.4/5.3 genesis/snapshot-tail parity and signed gap rejection | abstracted Task 3.1: snapshot contract | abstracted Task 3.2: gap/recovery | abstracted Task 3.3: recovery query | checked Tasks 2.4/5.3 snapshot+tail corpus | checked Task 5.3 boundary; forced network gap e2e Task 5.4 | planned syncing/conflict |
| `S-FAULT-006` | checked Tasks 2.1/5.4 strict 30,000-byte safe ceiling | N/A: byte size outside model | abstracted Task 3.2: oversize flag only | N/A: bytes outside query | checked Task 5.4 call/reply bounds | checked Task 5.4 released 32,768-byte API boundary; native near-limit Task 5.6 | planned safe error |
