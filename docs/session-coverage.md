...
| `S-ROOM-005` | checked Task 2.2 unready/cancel units | planned cancel assertion | checked Task 3.2 unready cancellation | queried Task 3.3 reverse-capable unready step | planned unready fixture | planned parity | planned unready/countdown |
...
| `S-ROOM-007` | checked Task 2.2 any-player abort | planned abort assertion | checked Task 3.2 abort-before-expiry | queried Task 3.3 abort successor/history | checked Task 2.4 abort transcript | planned multi-client | planned every-player control |
...
| `S-ROOM-010` | checked Task 2.2 actor/game gate | planned abstract game port | checked Task 3.2 abstract accepted action | queried Task 3.3 abstract action successors | checked Task 2.4 game-action transcript | planned parity | planned legal actions |
...
| `S-ROOM-016` | checked Task 2.2 terminal transition | planned phase assertion | checked Task 3.2 abstract post-game transition | queried Task 3.3 post-game successor | checked Task 2.4 postgame transcript | planned parity | planned results screen |
...
| `S-ROOM-019` | checked Task 2.2 transport-loss event | planned connection abstraction | checked Task 3.2 disconnect state | queried Task 3.3 route-loss step | checked Task 2.4 disconnect transcript | planned fault harness | planned reconnect status |
| `S-ROOM-020` | checked Task 2.2 stable-key reconnect | planned key-bound member | checked Task 3.2 durable-member reconnect | queried Task 3.3 reconnect relation | checked Task 2.4 reconnect transcript | planned restart e2e | planned reconnect status |
...
| `S-ROOM-022` | checked Task 2.2 absorbing close | planned absorbing-close | checked Task 3.2 `closed_is_absorbing` | queried Task 3.3 host/nonhost close policy | checked Task 2.4 close transcript | planned shutdown e2e | planned closed screen |
| `S-ROOM-023` | checked Task 2.2 host-loss/no-migration | planned single-host invariant | checked Task 3.2 fixed durable host; no migration | planned impossible migration query | planned unavailable event | planned host-loss e2e | planned unavailable message |
| `S-ROOM-024` | checked Task 2.2 bounded member/seat invariants | checked Task 3.1 member/seat separation and unique seat | checked Task 3.2 fixed two-seat scope | planned role query | planned snapshot fixture | planned parity | planned member/seat list |

## Authorization and integrity

| Rule | Rust | Alloy | NuSMV | Prolog | Protocol | Network | UI |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `S-AUTH-001` | checked Tasks 2.1-2.2 default-deny defect | checked Task 3.1 `DefaultDeny` | planned denied transition | queried Task 3.3 default-deny and outsider defect | planned unknown fixture | planned attack e2e | planned reason display |
...
| `S-AUTH-010` | checked Task 2.2 immutable policy evidence | planned decision completeness | planned decision outputs | queried Task 3.3 stable rule/explanation rows | planned decision fixture | planned audit log | planned policy reason |
...
| Rule | Rust | Alloy | NuSMV | Prolog | Protocol | Network | UI |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `S-VIEW-001` | checked Task 2.3 player projection/noninterference | checked Task 3.1 self-knowledge relation | planned visibility flag | queried Task 3.3 own-hand matrix | checked Task 2.3 typed own-hand field | planned private delivery | planned own-hand screen |
| `S-VIEW-002` | checked Task 2.3 public spectator projection | checked Task 3.1 no-grant/no-edge assertion | planned spectator flag | queried Task 3.3 public visibility matrix | checked Task 2.3 absent private fields | planned spectator capture | planned public-only screen |
| `S-VIEW-003` | checked Task 2.3 exact request unit | planned request relation | planned request state | queried Task 3.3 request successor/chain | checked Task 2.1 request schema | planned spectator e2e | planned request control |
| `S-VIEW-004` | checked Task 2.3 owner grant/deny/request/epoch gate | checked Task 3.1 owner/spectator grant relation | planned grant state | queried Task 3.3 grant explanations | checked Task 2.3 exact grant/deny schema | planned private e2e | planned owner grant control |
| `S-VIEW-005` | checked Task 2.3 exact revoke unit | checked Task 3.1 `RevocationStopsFutureKnowledge` | planned revoke state | queried Task 3.3 revoke explanations | checked Task 2.1 exact revoke schema | planned revoke e2e | planned owner revoke control |
| `S-VIEW-006` | checked Task 2.3 pairwise viewer test | checked Task 3.1 `ScopedSpectatorGrant` | planned grant flag | queried Task 3.3 exact can-see matrix | checked Task 2.3 scoped hand shape | planned recipient capture | planned granted hand |
| `S-VIEW-007` | checked Task 2.3 revoke/expiry future test | checked Task 3.1 future-edge absence | planned post-revoke state | queried Task 3.3 revoke produces false can-see | checked Task 2.3 projection epoch | planned capture after revoke | planned future removal |
...
| `S-VIEW-010` | checked Tasks 2.3/2.4 reconnect/snapshot hashes | checked Task 3.1 current-entitlement derivation | planned reconnect state | queried Task 3.3 visibility from current grant | checked Task 2.4 scoped snapshot projections | planned reconnect e2e | planned restored view |
...
| `S-VIEW-013` | checked Task 2.3 four-viewer controlled defect | checked Task 3.1 scoped-grant noninterference | planned visibility safety | queried Task 3.3 matrix and broadcast defect | checked Task 2.3 typed projection equality | planned multi-view capture | planned DOM/widget scan |
...
| Rule | Rust | Alloy | NuSMV | Prolog | Protocol | Network | UI |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `S-CHAT-001` | checked Task 2.2 member chat gate | planned permission assertion | planned chat action | queried Task 3.3 member chat policy | checked Task 2.4 chat transcript | planned e2e | planned composer |
...
| `S-CHAT-007` | checked Task 2.2 phase-preserving chat | planned phase permission | planned phase transitions | queried Task 3.3 phase-preserving self-loop | checked Task 2.4 running-phase fixture | planned e2e | planned multi-phase composer |
...
| Rule | Rust | Alloy | NuSMV | Prolog | Protocol | Network | UI |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `S-TIME-001` | checked Task 2.2 pure logical reducer | planned logical ordering | checked Task 3.2 logical command steps | planned event query | planned logical fields | planned runtime boundary | N/A: no wall clock authority |
| `S-TIME-002` | planned clock-adapter unit | N/A: external time abstracted | checked Task 3.2 eventual-expiry assumption | planned authority query | planned expiry fixture | planned timed harness | planned estimate only |
| `S-TIME-003` | checked Task 2.2 same-deadline race unit | planned abort-wins assertion | checked Task 3.2 abort-wins/stale-expiry safety | queried Task 3.3 abort/rearm history | planned race transcript | planned simultaneous harness | planned no false start |
...
| `S-TIME-006` | planned checker assumptions | checked Task 3.1 bounded safety only; exact scopes documented | checked Task 3.2 named conditional CTL/LTL assumptions | planned fairness explanation | planned evidence metadata | planned fault evidence | planned limitation text |
| `S-TIME-007` | checked Task 2.2 paused persistence/gate | checked Task 3.1 SAT pause/resume witness | checked Task 3.2 unconditional and missing-resume counterexamples | planned paused successor loop | N/A: claim metadata only | planned partition/pause harness | planned persistent paused state |
| `S-TIME-008` | checked Task 2.4 deterministic replay | planned logical steps | checked Task 3.2 deterministic scheduler traces | queried Task 3.3 three causal event histories | checked Task 2.4 transcript hashes | planned in-process only | planned replay parity |
...
| `S-FAULT-004` | checked Task 2.2 route-neutral reconnect state | planned stable-key abstraction | checked Task 3.2 route-neutral disconnect/reconnect | planned reconnect query | planned route metadata | planned rotation e2e | planned reconnect status |
...
| `S-FAULT-007` | checked Task 2.2 host-loss/no-election unit | checked Task 3.1 fixed singleton host/no election transition | checked Task 3.2 fixed-host liveness limitation | planned impossible-host query | planned unavailable event | planned host shutdown | planned host-unavailable screen |
| `S-FAULT-008` | planned evidence classification | N/A: network fact outside relational result | checked Task 3.2 unconditional persistence counterexample | planned availability query | planned disposition metadata | planned unavailable harness | planned honest limitation |
