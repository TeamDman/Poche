...
pub(super) fn terminal_if_no_peers(
    bytes: &[u8],
    creator: &poche_protocol::PrincipalId,
    ...
    let peers = room
        .recovery_peers(creator)
        .map_err(|_| "Saved lobby belongs to a different creator.")?;
    if peers.is_empty() {
        poche_veilid::DesktopRoomDisbanded::encode(genesis.room_id())
            .map(Some)
...
pub(super) fn restore(
    ...
    use poche_veilid::{DesktopRoomDisbanded, DesktopRoomGenesis, VeilidRendezvous};
    ...
    let invitation = genesis
        .invitation(now()?)
        .and_then(|code| {
            code.encode()
                .map_err(|_| DeviceClientError::ProtocolViolation)
        })
        .map_err(|_| "Saved invitation is invalid or expired.")?
        .expose()
        .to_owned();
    let identity = node
        .runtime()
    ...
    let adapter = VeilidRendezvous::new(node.api().clone())
        .map_err(|_| "Cannot initialize recovery routing.")?;
    let resumed = node
        .runtime()
        .block_on(adapter.resume_host_room(&identity, genesis.room_id(), u64::MAX, now()?))
    ...
    let routes = RunningHostRoute::start(
        node.clone(),
        adapter.own_resumed_route(resumed),
        route_updates,
    );
    let result = (|| {
        ...
        while !monitor
            .recovery_ready()
            .map_err(|_| "Recovery expired or persistence failed.")?
        {
            if started.elapsed() > Duration::from_secs(65) {
                return Err("No surviving peer answered recovery.");
            }
            std::thread::sleep(Duration::from_millis(100));
        }
        let (client, room_id) =
            poche_veilid::join_device(node.clone(), profile, store, &invitation, now()?)
                .map_err(|_| "The lobby recovered, but this device could not reconnect.")?;
        let mut live = NativeLiveDevice::connect(client, room_id)
            .map_err(|_| "Cannot load recovered table.")?;
