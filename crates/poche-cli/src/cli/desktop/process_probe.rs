...
#[cfg(all(windows, feature = "native-input-test"))]
#[test]
#[ignore = "test-only elevated broker must bind this process to a private clipboard station"]
fn protected_desktop_private_clipboard_role() {
    opt_in();
    let root = std::path::PathBuf::from(
        std::env::var_os("POCHE_PROCESS_PROBE_ROOT").expect("broker evidence directory"),
    );
    let role = std::env::var("POCHE_PROCESS_PROBE_ROLE").expect("broker role");
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        run_private_clipboard_role(&root, &role)
    }));
    let report = if result.is_ok() {
        "PASS: rendered role completed on the broker-attested private clipboard station.\r\n"
    } else {
        "FAIL: rendered role panicked; no invitation or clipboard content was persisted.\r\n"
    };
    fs::write(root.join(format!("{role}-rust-result.txt")), report)
        .expect("write redacted role result");
...
#[cfg(all(windows, feature = "native-input-test"))]
fn run_private_clipboard_role(root: &Path, role: &str) {
    use poche_native_ui::desktop_menu::input_probe::{
        self, MenuScenario, PrivateSystemClipboard,
    };

    assert!(matches!(role, "creator" | "joiner"), "unknown broker role");
    let guard = PrivateSystemClipboard::from_broker_environment(root, role)
        .expect("private clipboard broker attestation");
    let suffix = std::env::var("POCHE_PROCESS_PROBE_SUFFIX").expect("broker suffix");
    let evidence = std::path::PathBuf::from(
        std::env::var_os("POCHE_MENU_EVIDENCE_ROOT").expect("menu evidence root"),
    );
    let name = format!("clipboard-{role}-{suffix}");
    if role == "joiner" {
        wait_until(|| root.join("private-clipboard-ready").exists());
    }
    let coordination = root.to_owned();
    let scenario = if role == "creator" {
        MenuScenario::PrivateSystemClipboardCreate {
            clipboard_ready: Box::new(move || {
                fs::write(coordination.join("private-clipboard-ready"), b"ready")
                    .map_err(|_| "could not signal private clipboard readiness".to_owned())
            }),
        }
    } else {
        MenuScenario::PrivateSystemClipboardJoin
    };
    let mut connection = input_probe::run_private_system_clipboard(
        worker().expect("production connection worker"),
        validator(),
        &name,
        scenario,
        guard,
    ...
    .expect("rendered private OS clipboard input")
    .expect("private clipboard menu connected a live device");

    if role == "joiner" {
        assert!(
            connection.live.room_invitation().is_none(),
            "joiner unexpectedly owns the room invitation"
        );
        fs::write(root.join("private-clipboard-joined"), b"joined")
            .expect("signal clipboard join");
        wait_until(|| root.join("private-clipboard-observed").exists());
    } else {
        wait_until(|| {
            if !root.join("private-clipboard-joined").exists() {
                return false;
            }
            let _ = connection.live.poll();
            connection.live.observation().projection.payload.members.len() == 2
        });
        assert!(connection.live.room_invitation().is_some());
        fs::write(root.join("private-clipboard-observed"), b"observed")
            .expect("signal creator observation");
    }
}
